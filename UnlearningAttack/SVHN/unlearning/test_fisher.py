...
from fisher import fisher_information_martix
...
def load_CIFAR10_from_list(train_path=None, test_path=None, select_path=None):
    trainset = datasets.CIFAR10("./metrics/data", train=True, download=False, transform=torchvision.transforms.ToTensor())
    testset = datasets.CIFAR10("./metrics/data", train=False, download=False, transform=torchvision.transforms.ToTensor())
    selectset = datasets.CIFAR10(root='./metrics/data', train=True, download=False, transform=transforms.ToTensor())
...
if __name__ == "__main__":
    ...
    with open('../../sample_index_list/SVHN_2000.txt', 'rb') as f:
        select_list = pickle.load(f)
    ...
    train_dataset.data = np.concatenate([train_dataset.data[0:70000], X_original], axis=0)
    train_dataset.labels = np.concatenate([train_dataset.labels[0:70000], y], axis=0)
