...
if __name__ == "__main__":
    ...
    TA_set = torchvision.datasets.SVHN(
        root='../../data',
        split='test',
        download=True,
        transform=torchvision.transforms.ToTensor()
    ...
    MF_set = torchvision.datasets.SVHN(
        root='../../data',
        split='train',
        download=True,
        transform=torchvision.transforms.ToTensor()
    )
    MF_set.data = np.concatenate([train_dataset.data[0:70000], X_original], axis=0)
    MF_set.labels = np.concatenate([train_dataset.labels[0:70000], y], axis=0)
    MF_loader = torch.utils.data.DataLoader(MF_set, batch_size=48, shuffle=False, num_workers=16)

    MF_A_set = torchvision.datasets.SVHN(
        root='../../data',
        split='train',
        download=True,
        transform=torchvision.transforms.ToTensor()
    ...
    MF_A_set.labels = np.concatenate([y], axis=0)
    MF_A_loader = torch.utils.data.DataLoader(MF_A_set, batch_size=48, shuffle=False, num_workers=16)

    X_Ap = load_generated_samples(img_folder="../../selected_samples/SVHN/AEwith" + model_name, dataset='SVHN')
    Ap_set = torchvision.datasets.SVHN(
        root='../../data',
        split='train',
        download=True,
        transform=torchvision.transforms.ToTensor()
    )
    Ap_set.data = np.concatenate([X_Ap], axis=0)
    Ap_set.labels = np.concatenate([y], axis=0)
    ...
    train_dataset.labels = np.concatenate([y], axis=0)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=48, shuffle=True, num_workers=16)

    model.cuda()
    model.load_state_dict(torch.load('../models/'+ model_name +'_o.pth'))
    model = train(train_loader, TA_loader, model, epoch=2, save_path='',
                  RL=False, GA=True, lr=0.001)
