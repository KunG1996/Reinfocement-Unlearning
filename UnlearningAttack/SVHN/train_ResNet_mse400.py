...
from model_defination import ResNet18
...
# def train(trainloader, testloader, net, resume=False, check_point=None):
...
#             global_step += 1
#             writer.add_scalar("global_loss", loss, global_step=global_step)
#             loss.backward()
#             optimizer.step()
#
#             if i % 50 == 0:
#                 acc = test_in_train(testloader, net)
#                 print(epoch, acc)
#                 writer.add_scalar("global_acc", acc, global_step=global_step)
#
#         if epoch % 10 == 0:
#             save_model(net, './models/ResNet18_CIFAR10_' + str(epoch) + '.pth')
#
#         # print statistics
#     save_model(net, './models/ResNet18_CIFAR10.pth')
...
if __name__ == "__main__":
    ...
    model = train(train_loader, test_loader, model, record='./runs/'+  model_name +'_o&g(mse400)',
                  save_path='./models/' + model_name+ '_o&g(mse400)_' ,epoch=300, RL=False, GA=False)
    save_model(model, './models/'+ model_name +'_o&g(mse400).pth')
